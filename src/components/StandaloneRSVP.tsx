        ...
        {!successGuest ? (
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="space-y-5 font-sans">
              ...
              {/* Companion Counter Selector */}
              <div>
                <label className="block text-gold-800 text-xs uppercase tracking-widest font-bold mb-2.5">
                    ...
                    <button
                      ...
                      className={`flex-1 py-2.5 rounded-xl border text-xs font-bold cursor-pointer transition-all ${
                        companionCount === num
                          ...
                          : 'bg-white/80 border-gold-200 text-gold-700 hover:bg-gold-100/50'
                      }`}
                    >
                      {num}
                    </button>
              ...
              {/* Companion Inputs (Dynamic Fields) */}
              {companionCount > 0 && (
                <motion.div 
                  initial={{ opacity: 0, height: 0 }}
                  animate={{ opacity: 1, height: 'auto' }}
                  className="space-y-3 bg-gold-50/50 border border-gold-200/50 p-4 rounded-2xl overflow-hidden"
                >
                  <p className="text-gold-700 text-[11px] uppercase tracking-wider mb-1 flex items-center gap-1.5 font-bold">
                    <Users size={14} className="text-gold-500" />
                    Nome dos Acompanhantes
                  </p>
                  {companions.map((comp, idx) => (
                    <div key={idx} className="relative">
                      <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gold-500 text-xs font-bold">
                        #{idx + 1}
                      </span>
                      <input
                        type="text"
                        ...
                        disabled={loading}
                        className="w-full bg-white border border-gold-200 rounded-xl py-2.5 pl-9 pr-3 text-gold-950 text-xs font-medium focus:outline-none focus:border-gold-500"
                      />
                    </div>
                  ))}
                </motion.div>
              ...
              <div>
                <label className="block text-gold-800 text-xs uppercase tracking-widest font-bold mb-2">
                  Restrições Alimentares ou Alergias (Opcional)
                </label>
                <input
                  type="text"
                  placeholder="Ex: Vegetariano, Vegano, Alergia a frutos do mar, etc."
